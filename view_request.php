<?php
    ...
    include('connection.php');              /* connection file include here for connection to the database */
?>
<html>
	<head>
	  <link href="css/frame.css" rel="stylesheet">   <!--here we include css file for styleing the form-->
	  <link href="css/frame_menu.css" rel="stylesheet"> <!--here we include css file for menu style-->
	</head>
 <body>
	...
	<script>
		/* this function is used for the responsivness of the page*/
		 function myFunction() {
 ...
 <?php 
	...
	class betterhalf_user{
		/* function view request display the all the request of corresponding user*/
		public function view_request($conn)
		{
		 /*here we select all the request coresponding user which is pending*/
		     ...
		     while($senders = $total_request_res->fetch_array(MYSQLI_BOTH))
			  {
		        ...
		        {
			     if($senders_details== '.' || $senders_details=='..') continue;           //it is used for image
			      {
			      }
...
<?php 	        }
	   ...
	   $view_request=new betterhalf_user();      /*view_request is a object of betterhalf class*/
	   $view_request->view_request($conn);       /*here we call the view request function by view request object*/
